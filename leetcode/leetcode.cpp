...
string leet_code_solution::largestNumber(vector<int> &num){
	...
	for (int i = 0; i < num.size(); i++){
		v_str.push_back(to_string(num.at(i)));
		sum += num.at(i);
	}
	if (0 == sum)
		return "0";
	sort(v_str.begin(), v_str.end(), compar_str);
...
int leet_code_solution::my_reverse_int(int x){
	...
	for (int i = 0; i < s.size(); i++){
		if ((new_int > _INT_MAX / 10) || (sign > 0 ? (new_int == _INT_MAX / 10 && s.at(i) > INT_MAX % 10) : (s.at(i) == ((unsigned int)_INT_MAX + 1) / 10 && s.at(i) > (((unsigned int)_INT_MAX + 1) % 10)))){
			return 0;
		}
		new_int = new_int * 10 + s.at(i);
	}
	return sign * new_int;
}
/*Question:
...
//private:
bool leet_code_solution::compar_str(string str1, string str2){
